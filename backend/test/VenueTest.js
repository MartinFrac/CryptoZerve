...
contract("Venue", (accounts) => {
  ...
  it("Check if only gas is used", async () => {
    ...
    assert.equal(
      ...
      balanceAfter.add(gasPrice.mul(gasUsed)).toString()
    );
  });
  ...
  it("Accept offer should send money to the contract", async () => {
    ...
    const data = await venue.acceptOffer(0, {
      from: accounts[1],
      value: price,
    });
    ...
    assert.equal(
      balanceBefore.toString(),
      balanceAfter.add(toBN(price).add(gasUsed.mul(gasPrice))).toString()
    );
    assert.equal(
      venueBalanceAfter.toString(),
      venueBalanceBefore.add(toBN(price)).toString()
    );
  });
});
